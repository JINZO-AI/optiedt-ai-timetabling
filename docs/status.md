...
| | |
|---|---|
| **Current phase** | **Phase 6 — tests, documentation, presentation. UNDER WAY**, M0-M4 of 8 done. **Seven of nine acceptance criteria met.** Phase 5 closed 2026-08-04: six milestones and a closing audit. Milestone table in [`docs/dashboard.md`](dashboard.md) |
...
| **Next step** | **Phase 6 M5 — the instance generator** (`data/generator/`, owed since PPM §10), then M6 the demonstration script and the timed FR-2 walkthrough, then M7 documentation and the closing audit. ⚠️ **Criterion 5 needs a project-owner decision**: M3 measured it as met on infeasibilities CP-SAT can prove and **not** met on the C-13 contiguity shape, where the run lands in `FAILED` and the pre-analysis names the resource instead |
