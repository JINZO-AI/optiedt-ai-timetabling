...
| | |
|---|---|
| **Current phase** | **Phase 5 — pre-analysis in-app, diagnosis, authentication, run record. IN PROGRESS, M4 of 6 done.** Phase 4 closed 2026-08-01: six milestones and a closing audit. Milestone table in [`docs/dashboard.md`](dashboard.md) |
...
| **Next step** | **Phase 5 M5 — publication and traceability**. M1 landed FR-12 with both bounds; M2 landed FR-8 and resolved C-17; M3 landed FR-19 against real PostgreSQL; M4 landed FR-11 and resolved C-18. Then M6, the closing audit |
...
|---|---|---|---|
| **Toolchain** | **all green** | 2026-08-04 | **10/10** layer contracts kept · ruff · format · mypy strict on **69** source files · **271 backend tests** (219 fast + 22 solver-marked + 30 database-marked) · instance verified · frontend `tsc` clean · **19 frontend tests** (`vitest`). `run-checks.ps1` now has **nine** steps: the database step FAILS when Docker is up but the container is not, and SKIPS when Docker is absent. Phase 5 M1 added 32 backend and 5 frontend; M2 added 13 backend and 6 frontend; M3 added 30 backend. Ninth contract (`api ⇸ solver`) added in Phase 4 M1 and verified to fire; `pytest` exit 5 no longer tolerated |
