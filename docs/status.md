...
| | |
|---|---|
| **Current phase** | **Phase 6 — tests, documentation, presentation. UNDER WAY**, M0-M3 of 8 done. Phase 5 closed 2026-08-04: six milestones and a closing audit. Milestone table in [`docs/dashboard.md`](dashboard.md) |
...
| **Next step** | **Phase 6 M4 — the half-day closure (criterion 7, FR-9).** ⚠️ **Criterion 5 needs a project-owner decision**: M3 measured it as met on infeasibilities CP-SAT can prove and not met on the C-13 contiguity shape, where the run lands in FAILED and the pre-analysis names the resource instead. ✅ **C-5 and C-14 were settled 2026-08-05** by project-owner decision, and **C-9 does not block the suite** (corrected: neither the nine criteria nor `testing-strategy.md` §4 names FR-6, FR-10, FR-17 or FR-18). Also here: the **instance generator** (owed, PPM §10), the **timed FR-2 walkthrough**, and the three unmet acceptance criteria |
