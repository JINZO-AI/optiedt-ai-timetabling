...
|---|---|---|---|
| **Toolchain** | **all green** | 2026-08-04 | **10/10** layer contracts kept · ruff · format · mypy strict on **71** source files · **287 backend tests** (227 fast + 22 solver-marked + 38 database-marked) · instance verified · frontend `tsc` clean · **24 frontend tests** (`vitest`). `run-checks.ps1` now has **nine** steps: the database step FAILS when Docker is up but the container is not, and SKIPS when Docker is absent. Phase 5 M1 added 32 backend and 5 frontend; M2 added 13 backend and 6 frontend; M3 added 30 backend. Ninth contract (`api ⇸ solver`) added in Phase 4 M1 and verified to fire; `pytest` exit 5 no longer tolerated |
