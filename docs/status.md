...
| | |
|---|---|
| **Current phase** | **Phase 4 — the web interface. In progress, M3 of 6 complete.** The API is complete for this phase — instance, FR-2 availability, run lifecycle, candidates, comparison, dominance, recommendation — and the generation screen runs end to end against the real solver. Milestone table in [`docs/dashboard.md`](dashboard.md) |
...
| **Next step** | Phase 4 **M4** — the four timetable views (FR-7, FR-18). ⚠️ Settle **C-14** before M5's dominance signal and **C-12(a)** before M6's grid cell model |
...
- [ ] The sum of displayed contributions equals the score difference, to display precision — **computation verified** 2026-07-30 on three real portfolio candidates, agreeing to 9 decimal places, plus a hypothesis property test. Left unticked because *displayed* needs the Phase 4 comparison screen
