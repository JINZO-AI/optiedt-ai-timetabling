...
| **Next task** | **Phase 6 M2 — the acceptance harness**: `backend/tests/acceptance/`, one file per FR, driven through the API so each proves *reachable* capability, covering the six criteria already met. ✅ **C-5 and C-14 were settled 2026-08-05** and C-9 was found not to block the suite, so nothing gates the acceptance tests. Still to land: the **instance generator** (a stated PPM §10 deliverable that does not exist), the **timed FR-2 walkthrough**, and the three unmet acceptance criteria |
...
**Status.** M0 and M1 done. **Six of nine** acceptance criteria are met — Phase 5 added **two**: a
teacher's own data (M4) and a published timetable's trace (M5).
