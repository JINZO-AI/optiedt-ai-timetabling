...
> sequence**, which is the project owner's tracking view. **Currently Phase 10 of 14.** This page stays the
> handoff page and the traceability table stays the authority on any single requirement.

**Last updated 2026-08-10. Phase 9 is COMPLETE — FR-10's software, print and export on all four views.**

⚠️ **Phase 9 delivered a requirement and ticked nothing, which is the correct outcome and worth reading
before quoting any count.** Print and CSV export are reachable from every timetable view and covered by
28 new display tests; **FR-10 stays `WIP`** because C-9 leaves it with no criterion to verify against.
**Software status and requirement status are different things here** — the `✓` count is unchanged at 10
of 25, and inflating it would be the failure this page exists to prevent.
...
| **Overall progress** | **Increment 1 is COMPLETE.** **Nine phases delivered** — the plan's six (20 days), Phase 7 for the work C-1 recorded and no phase carried, Phase 8 for increment-1 closure, and **Phase 9, outputs and distribution, on 2026-08-10**. ⚠️ **The continuous phase view is [`docs/project-roadmap.md`](project-roadmap.md); we are now in Phase 10 of 14.** By *delivered product*: **all 9 of 9 acceptance criteria** met (the ninth on 2026-08-07, ⚠️ **reworded** — no time was measured), and **10 of 25 requirements `✓`, 12 under way, 3 not started** — a requirement is `✓` only once a user can reach it *and* it is tested end to end. ⚠️ **Phase 9 moved FR-10 from `—` to `WIP` and moved the `✓` count by nothing**, because C-9 holds its tick; that is the honest bookkeeping, not an omission. ⚠️ **This row said "75 %, Phases 1–4 delivered, Phase 5 under way, 0 requirements finished" until 2026-08-06** — false since 2026-08-04 and 2026-08-06 respectively, and missed by two closing audits. Both numbers are real; quote the measure with the number |
| **Current phase** | **Phase 10 — Requirement closure by test. NOT STARTED.** Phase 9 closed on 2026-08-10. ⚠️ **This row said "Phase 7" until 2026-08-07** and was stale for two phases; the continuous roadmap is now the tracking view — [`docs/project-roadmap.md`](project-roadmap.md) |
| **Last completed phase** | **Phase 9 — COMPLETE 2026-08-10**: FR-10's software — a `@media print` stylesheet with a print-only identity header, and a CSV export carrying the run's whole trace, on all four timetable views and on publications. **No backend file was touched.** Before it, **Phase 8 — COMPLETE 2026-08-07**: the ninth acceptance criterion (FR-2), FR-24's live provider call, **C-15 resolved on measurement** (FR-13 → `✓`) and the production secret-key guard. Before it, **Phase 7 — COMPLETE 2026-08-06**, regeneration and the assistant. Before that, **Phase 6 — COMPLETE 2026-08-06**, eight milestones and a closing audit: the acceptance suite, the instance generator, the demonstration script, C-5 and C-14 implemented. Before it, Phase 5 delivered pre-analysis in-app, the diagnosis run, the run record, authentication and publication |
| **Milestone reached** | **Phase 9's, 2026-08-10** — *a timetable can leave the screen*: any of the four views prints as an identified sheet or downloads as a spreadsheet, both carrying the run, seed and full weight vector that produced them. Before it, Phase 7's, 2026-08-06 — *increment 1 closed* |
| **Current goal** | **Phase 10 — close by test the five requirements that lack only an acceptance file**: FR-3, FR-4, FR-7, FR-14, FR-16. Pure evidence, no new software; it would take the count from 10 `✓` to 15. ⚠️ Increment 2 (Phases 13–14) remains *conditional on remaining time*. Natural-language constraint entry is **not undertaken** (`docs/ai-integration.md`) |
| **Next task** | **Code, for the first time in three phases**: `tests/acceptance/test_fr03…16`, one file per requirement, driven through the HTTP API, each opening with the criterion it verifies. ⚠️ **C-9 remains open** and now blocks **four** requirement ticks (FR-6, FR-10, FR-17, FR-18 — `open-questions.md` records three, excluding FR-17; see the note under Open questions below). It is supervisor-dependent and **cannot be closed from this repository.** Everything else that was outstanding is settled: the FR-2 walkthrough (2026-08-07, criterion reworded), the first live provider call (2026-08-07, FR-24 `✓`), C-15 (2026-08-07, FR-13 `✓`) |
...
| **Project health** | 🟢 **Green.** `scripts/run-checks.ps1` green across **nine** steps: **517 backend tests + 81 frontend**, **11/11 layer contracts** kept, mypy strict on **78** files, instance verified, **42 store-contract tests against real PostgreSQL** and **2 migration tests**. ⚠️ **This row read "510 backend + 53 frontend" until 2026-08-10 and was stale by Phase 8's own additions** — `test_fr02` (5) and `test_config_guard` (7). **Derive these rather than editing them**: the collected total is the first line of `uv run pytest --collect-only -q | Select-Object -Last 1`, and the roadmap's §7 had the right figure while this row did not. Separately, `scripts/run-acceptance.ps1` green: **112 acceptance tests over 15 requirements** (FR-2, 3, 5, 8, 9, 11, 12, 13, 15, 17, 19, 22, 23, 24, 25), of which **36 solve for real**. ⚠️ **This read "106 over 14 … 35" until 2026-08-10 and was stale by Phase 8's own `test_fr02`.** **Derive it**: `ls backend/tests/acceptance/test_fr*.py` for the requirements, and `uv run pytest tests/acceptance --collect-only -q` for the count. **9 of 9 acceptance criteria met**; ⚠️ the ninth **reworded** 2026-08-07, no time measured. The engine is validated on all 21 published ITC-2007 instances, and the whole path — declare availability, generate, compare, regenerate, explain, publish — is verified against the real solver |
...
⚠️ **The bars above stop being an increment-1 picture at Phase 8.** Phases 8 and 9 are shown because a
reader arriving here should not have to open a second file to learn that two more phases have closed —
...
one ruling. Raised here because the counts in the two files will keep diverging by one until it is made.
