...
| **Next task** | **Phase 6 M5 — the instance generator** (`data/generator/`), a stated PPM §10 deliverable that does not exist. It reproduces every documented figure without touching `data/instance/` (project-owner decision, 2026-08-05). Then **M6** the demonstration script and the timed FR-2 walkthrough, **M7** documentation and the closing audit. ⚠️ **Criterion 5 needs a decision** — M3 measured it as met on one shape of infeasibility and not the other |
