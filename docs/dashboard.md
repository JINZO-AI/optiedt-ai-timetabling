...
> sequence**, which is the project owner's tracking view. **Phases 1–10 are complete; Phase 11 of 14 is
...
**Last updated 2026-08-10. Phase 10 is COMPLETE — five requirements closed by test, 10 `✓` → 15 of 25.**
...
| **Project health** | 🟢 **Green.** `scripts/run-checks.ps1` green across **nine** steps: **552 backend tests + 99 frontend**, **11/11 layer contracts** kept, mypy strict on **78** files, instance verified, **42 store-contract tests against real PostgreSQL** and **2 migration tests**. ⚠️ **This row read "517 backend + 81 frontend" until 2026-08-10 and "510 + 53" before that** — stale twice, by two different phases' own additions. **Derive these rather than editing them**: the collected total is the last line of `uv run pytest --collect-only -q`, and `npm run test` prints the frontend's. Separately, `scripts/run-acceptance.ps1` green: **147 acceptance tests over 19 requirements** (FR-2, 3, 4, 5, 7, 8, 9, 11, 12, 13, 14, 15, 16, 17, 19, 22, 23, 24, 25), of which **41 solve for real**. ⚠️ **This read "112 over 15 … 36" until Phase 10.** **Derive it**: `ls backend/tests/acceptance/test_fr*.py` for the requirements, and `uv run pytest tests/acceptance --collect-only -q` for the count. **9 of 9 acceptance criteria met**; ⚠️ the ninth **reworded** 2026-08-07, no time measured. The engine is validated on all 21 published ITC-2007 instances, and the whole path — declare availability, generate, compare, regenerate, explain, publish — is verified against the real solver |
...
⚠️ **The bars above stop being an increment-1 picture at Phase 8.** Phases 8, 9 and 10 are shown because
a reader arriving here should not have to open a second file to learn that three more phases have closed —
