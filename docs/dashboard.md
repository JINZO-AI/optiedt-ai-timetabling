...
**Last updated 2026-08-11. Phase 11 is COMPLETE — the administrative surfaces are built, 16 `✓` → 18 of 25.**
...
| **Next task** | **A decision: whether to open Phase 12.** Nothing in the repository is blocked, and the phase's brief is written. ⚠️ **C-9 remains open over two requirements — FR-10 and FR-18** — and is supervisor-dependent. FR-18's is the sharper gap: **no document defines what the occupancy figure is**, and C-13 is this project's record of losing three sessions to that exact choice, so it is the one place a session must not invent. ⚠️ **Two small pieces of work are unscheduled and deliberately not assigned.** **(1) FR-6's acceptance file** — its criterion is SRS §6.7, quoted in `testing-strategy.md` §4; the behaviour is implemented and reachable; only the file is missing. **(2) The shortened-day shift in the timetable views** — Phase 11's recorded limitation: the window is configured and previewed, and the printed grid still shows ordinary hours. Assigning either to a phase is the project owner's call. Everything else that was outstanding is settled: the FR-2 walkthrough (2026-08-07), the first live provider call (2026-08-07), C-15 (2026-08-07), FR-17 (2026-08-10), FR-9 and FR-11 (2026-08-11) |
...
| **Project health** | 🟢 **Green.** `scripts/run-checks.ps1` green across **nine** steps: **632 backend tests + 125 frontend**, **11/11 layer contracts** kept, mypy strict on **83** files, instance verified, **60 store-contract tests against real PostgreSQL** and **2 migration tests**. ⚠️ **This row read "552 backend + 99 frontend" until 2026-08-11, "517 + 81" before that and "510 + 53" before that** — stale three times, by three different phases' own additions. **Derive these rather than editing them**: the collected total is the last line of `uv run pytest --collect-only -q`, and `npm run test` prints the frontend's. Separately, `scripts/run-acceptance.ps1` green: **182 acceptance tests over 19 requirements plus the student surface** (FR-2, 3, 4, 5, 7, 8, 9, 11, 12, 13, 14, 15, 16, 17, 19, 22, 23, 24, 25, and `test_student_view`), of which **40 solve for real** — measured, not estimated: **21 min 19 s on 2026-08-11**, inside the recorded range. ⚠️ **The solver-marked count went DOWN, 41 → 40, while the total went up**: `test_fr09` carried a module-level `solver` mark over five tests, and Phase 11 moved it onto the four that actually need CP-SAT. ⚠️ **This read "147 over 19 … 41" until Phase 11 and "112 over 15 … 36" until Phase 10.** **Derive it**: `ls backend/tests/acceptance/test_fr*.py` for the requirements, and `uv run pytest tests/acceptance --collect-only -q` for the count. **9 of 9 acceptance criteria met**; ⚠️ the ninth **reworded** 2026-08-07, no time measured. The engine is validated on all 21 published ITC-2007 instances, and the whole path — declare availability, generate, compare, regenerate, explain, publish — is verified against the real solver |
