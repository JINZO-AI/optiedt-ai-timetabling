...
> sequence**, which is the project owner's tracking view. **Phases 1–11 are complete; Phase 12 of 14 is
> next and has not begun.** This page stays the handoff page and the traceability table stays the
> authority on any single requirement.

**Last updated 2026-08-11. Phase 11 is COMPLETE and C-9 is CLOSED — 16 `✓` → 21 of 25, and every open
question in this project is now resolved.**
...
| **Overall progress** | **Increment 1 is COMPLETE.** **Eleven phases delivered** — the plan's six (20 days), Phase 7 for the work C-1 recorded and no phase carried, Phase 8 for increment-1 closure, **Phase 9** (outputs and distribution) and **Phase 10** (requirement closure by test), both 2026-08-10, and **Phase 11** (administrative surfaces) on 2026-08-11. ⚠️ **The continuous phase view is [`docs/project-roadmap.md`](project-roadmap.md); Phase 12 of 14 is next and has not begun.** By *delivered product*: **all 9 of 9 acceptance criteria** met (the ninth on 2026-08-07, ⚠️ **reworded** — no time was measured), and **21 of 25 requirements `✓`, 1 under way, 3 not started** — a requirement is `✓` only once a user can reach it *and* it is tested end to end. ⚠️ **The single remaining `WIP` is FR-8**, held by a statement that would have to be narrowed to match what the software can do — and this project refuses that direction. **Nothing is held by an unbuilt screen or by an open question any longer; C-9 closed on 2026-08-11.** ⚠️ **This row said "75 %, Phases 1–4 delivered, Phase 5 under way, 0 requirements finished" until 2026-08-06** — false since 2026-08-04 and 2026-08-06 respectively, and missed by two closing audits. Both numbers are real; quote the measure with the number |
| **Current phase** | **Phase 12 — Data management (FR-1). NOT STARTED**, and **opening it is the project owner's call, not a session's.** Phase 11 closed on 2026-08-11. ⚠️ **This row said "Phase 7" until 2026-08-07** and was stale for two phases; the continuous roadmap is now the tracking view — [`docs/project-roadmap.md`](project-roadmap.md) |
| **Last completed phase** | **Phase 11 — COMPLETE 2026-08-11**: the administrative surfaces. **FR-9 and FR-11 closed by building them, 16 `✓` → 18.** Six new endpoints — the calendar, the accounts and the student's own timetable — a `calendar_overrides` table layered over the pristine instance, `users.group`, one migration, and the role restriction that keeps a student off every other timetable endpoint. **80 backend tests and 26 frontend; 17 of 17 mutations detected.** ⚠️ **One limitation recorded**: the shortened-day shift is configured and previewed, not carried into the timetable views. Before it, **Phase 10 — COMPLETE 2026-08-10**: FR-3, FR-4, FR-7, FR-14 and FR-16 closed by test, **10 `✓` → 15**. `acceptance/test_fr03` rewritten to cover **all twelve H codes** (it checked seven and named two of them by the wrong code); new `test_fr04`, `test_fr07`, `test_fr14`, `test_fr16`; and two frontend files for FR-7's display half. **One source file touched — a stale comment.** Before it, **Phase 9 — COMPLETE 2026-08-10**: FR-10's software — a `@media print` stylesheet with a print-only identity header, and a CSV export carrying the run's whole trace, on all four timetable views and on publications. **No backend file was touched.** Before it, **Phase 8 — COMPLETE 2026-08-07**: the ninth acceptance criterion (FR-2), FR-24's live provider call, **C-15 resolved on measurement** (FR-13 → `✓`) and the production secret-key guard. Before it, **Phase 7 — COMPLETE 2026-08-06**, regeneration and the assistant. Before that, **Phase 6 — COMPLETE 2026-08-06**, eight milestones and a closing audit: the acceptance suite, the instance generator, the demonstration script, C-5 and C-14 implemented. Before it, Phase 5 delivered pre-analysis in-app, the diagnosis run, the run record, authentication and publication |
| **Milestone reached** | **Phase 11's, 2026-08-11** — *every actor SRS Table 2 names now has the surface it names*. The person in charge, the teacher, the administrator and the student each reach their own, and each is refused the others'. What remains is held by a document the supervisor must write (2), a wording (1) and one unwritten acceptance file (1) — **no requirement is now held by an unbuilt screen.** Before it, the pre-Phase-11 audit's, 2026-08-10 — *every requirement that repository evidence can close is closed* |
| **Current goal** | **Phase 12 — data management (FR-1). It has not started, and opening it is the project owner's call.** ⚠️ **Read [`docs/project-roadmap.md`](project-roadmap.md) §5's Phase 12 subsection before writing a line.** FR-1 is the last requirement with no software outside the two conditional phases: the only way in is 13 hand-authored CSVs, so no second institution can use the application. ⚠️ **Scope discipline is the risk there**, as it was here: the solver decides `(slot, room)` only, and an import path must not grow into a student information system. ⚠️ Increment 2 (Phases 13–14) remains *conditional on remaining time*. Natural-language constraint entry is **not undertaken** (`docs/ai-integration.md`) |
| **Next task** | **A decision: whether to open Phase 12.** Nothing in the repository is blocked, no open question remains, and the phase's brief is written. ⚠️ **One small piece of work is unscheduled and deliberately not assigned: the shortened-day shift in the timetable views** — Phase 11's recorded limitation, where the window is configured and previewed and the printed grid still shows ordinary hours. ⚠️ **And one requirement, FR-8, is `WIP` by decision rather than by omission**: promoting it would need its criterion narrowed to match a limitation, which this project refuses. Assigning either to a phase is the project owner's call. Everything else that was outstanding is settled: the FR-2 walkthrough (2026-08-07), the first live provider call (2026-08-07), C-15 (2026-08-07), FR-17 (2026-08-10), FR-9 and FR-11 (2026-08-11) |
...
| **Project health** | 🟢 **Green.** `scripts/run-checks.ps1` green across **nine** steps: **647 backend tests + 142 frontend**, **11/11 layer contracts** kept, mypy strict on **83** files, instance verified, **60 store-contract tests against real PostgreSQL** and **2 migration tests**. ⚠️ **This row read "632 + 125" earlier on 2026-08-11, "552 + 99" before that, "517 + 81" before that and "510 + 53" before that** — stale four times, by four different pieces of work adding their own tests. **Derive these rather than editing them**: the collected total is the last line of `uv run pytest --collect-only -q`, and `npm run test` prints the frontend's. Separately, `scripts/run-acceptance.ps1` green: **197 acceptance tests over 21 requirements plus the student surface** (FR-2, 3, 4, 5, **6**, 7, 8, 9, 11, 12, 13, 14, 15, 16, 17, **18**, 19, 22, 23, 24, 25, and `test_student_view`), of which **40 solve for real** — measured, not estimated: **19 min 01 s on 2026-08-11**, inside the recorded range. ⚠️ **The solver-marked count went DOWN, 41 → 40, while the total went up**: `test_fr09` carried a module-level `solver` mark over five tests, and Phase 11 moved it onto the four that actually need CP-SAT. ⚠️ **This read "147 over 19 … 41" until Phase 11 and "112 over 15 … 36" until Phase 10.** **Derive it**: `ls backend/tests/acceptance/test_fr*.py` for the requirements, and `uv run pytest tests/acceptance --collect-only -q` for the count. **9 of 9 acceptance criteria met**; ⚠️ the ninth **reworded** 2026-08-07, no time measured. The engine is validated on all 21 published ITC-2007 instances, and the whole path — declare availability, generate, compare, regenerate, explain, publish — is verified against the real solver |
...
✅ **All twenty are now resolved — none is open.** ⚠️ **That is not the same as "the specification
answered everything."** Six were settled as **project decisions from repository evidence** because the
supervisor was unavailable (C-5, C-14, C-19, C-20, C-21, and C-9 in two stages), each recording what
would reverse it. Read a clear table as *decided and documented*, never as *confirmed by the
supervisor*.
