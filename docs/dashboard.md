...
| **Current phase** | **Phase 4 — the web interface. IN PROGRESS, M2 of 6 complete.** The API is complete for this phase; frontend still scaffold only. Milestone table in the Phase 4 roadmap block below |
...
| **Next task** | Phase 4 **M3** — the frontend shell and the generation screen (FR-13, FR-5, FR-6): first React code, launch a run and poll it, show ranked candidates with their seven sub-scores. ⚠️ Settle **C-14** before M5's dominance signal and **C-12(a)** before M6's grid cell model |
